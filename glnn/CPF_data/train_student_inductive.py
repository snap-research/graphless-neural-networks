...
from utils import get_logger, get_evaluator, set_seed, get_training_config, check_writable, check_readable, graph_split, compute_min_cut_loss
...
def run(args):
    ...
    out_t_dir = Path.cwd().joinpath(args.output_path, 'inductive', f'split_rate_{args.split_rate}', args.dataset, args.teacher, f'seed_{args.seed}')
    ...
    obs_idx_t = torch.cat([obs_idx_train, obs_idx_test])
    ...
    '''Run'''
    loss_and_score = []
    out, score_val, score_test_tran, score_test_ind = distill_run_inductive(conf, model, g, feats, labels, out_t, distill_indices, criterion_l, criterion_t, evaluator, optimizer, logger, loss_and_score)
    logger.info(f"Teacher: {conf['teacher']}. Student: {conf['student']}. Dataset: {conf['dataset']}. Split rate: {conf['split_rate']}. Lamb: {conf['lamb']}")
