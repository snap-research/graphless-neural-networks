...
from utils import get_logger, get_evaluator, set_seed, get_training_config, check_writable, check_readable, compute_min_cut_loss, graph_split, feature_prop
...
def run(args):
    ...
    if args.exp_setting == 'tran':
        ...
        output_dir = Path.cwd().joinpath(args.output_path, 'inductive', f'split_rate_{args.split_rate}', args.dataset, f'{args.teacher}_{args.student}', f'seed_{args.seed}')
        out_t_dir = Path.cwd().joinpath(args.out_t_path, 'inductive', f'split_rate_{args.split_rate}', args.dataset, args.teacher, f'seed_{args.seed}')
    else:
    ...
    g, labels, idx_train, idx_val, idx_test = load_data(args.dataset, args.data_path, split_idx=args.split_idx, seed=args.seed, labelrate_train=args.labelrate_train, labelrate_val=args.labelrate_val)
    ...
    if args.exp_setting == 'tran':
        ...
        out, score_val, score_test = distill_run_transductive(conf, model, feats, labels, out_t, distill_indices, criterion_l, criterion_t, evaluator, optimizer, logger, loss_and_score)
        score_lst = [score_test]

    elif args.exp_setting == 'ind':
        # Create inductive split
        obs_idx_train, obs_idx_val, obs_idx_test, idx_obs, idx_test_ind = graph_split(idx_train, idx_val, idx_test, args.split_rate, args.seed)
        obs_idx_l = obs_idx_train
        obs_idx_t = torch.cat([obs_idx_train, obs_idx_val, obs_idx_test])
        distill_indices = (obs_idx_l, obs_idx_t, obs_idx_val, obs_idx_test, idx_obs, idx_test_ind)
        ...
        out, score_val, score_test_tran, score_test_ind = distill_run_inductive(conf, model, feats, labels, out_t, distill_indices, criterion_l, criterion_t, evaluator, optimizer, logger, loss_and_score)
...
def main():
    ...
    if args.num_exp == 1:
        ...
        score_str = ''.join([f'{s : .4f}\t' for s in score_mean] + [f'{s : .4f}\t' for s in score_std])
