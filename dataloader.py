...
from data_preprocess import normalize_adj, eliminate_self_loops_adj, largest_connected_components, binarize_labels
...
def load_data(dataset, dataset_path, **kwargs):
    if dataset in CPF_data:
        return load_cpf_data(dataset, dataset_path, kwargs['seed'], kwargs['labelrate_train'], kwargs['labelrate_val'])
...
def load_nonhom_data(dataset, dataset_path, split_idx):
    ...
    data_split_path = Path.cwd().joinpath(dataset_path, 'splits', f'{dataset}-splits.npy')
...
def load_bgnn_data(dataset, dataset_path, split_idx):
    ...
    train_mask, val_mask, test_mask = masks[str(split_idx)]['train'], masks[str(split_idx)]['val'], masks[str(split_idx)]['test']
...
def load_pokec_mat(data_path):
    if not path.exists(data_path):
        gdd.download_file_from_google_drive(
            ...
            dest_path=data_path, showsize=True)
...
def load_fixed_splits(dataset, data_split_path='', split_idx=0):
    if not os.path.exists(data_split_path):
        ...
        gdd.download_file_from_google_drive(
            ...
            dest_path=data_split_path, showsize=True)
